      ...
      <div className="relative space-y-4">
        ...
        <div className="flex gap-2">
          <div className="relative flex-1">
            <User className="absolute z-50 left-2.5 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
            <Input
              placeholder="Enter username"
              value={username}
              onChange={(e) => { setUsername(e.target.value); setError(""); }}
              onKeyDown={handleKeyDown}
              className="h-9 rounded-lg border-slate-200/80 bg-white/80 pl-8 pr-3 text-xs backdrop-blur-sm focus:border-pink-400 focus:ring-2 focus:ring-pink-100 dark:border-slate-700/80 dark:bg-slate-800/80 dark:text-slate-300"
            />
          </div>
          <Button
