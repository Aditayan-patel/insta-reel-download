...
export function InstagramThumbnailDownloader() {
  ...
  const handleFetchThumbnail = async () => {
    ...
    if (!shortcode) { toast.error("Invalid Instagram URL"); return; }
      ...
      if (data.data?.xdt_shortcode_media) {
        ...
        if (media.carousel_media) {
          media.carousel_media.forEach((item: any) => {
            if (item.image_versions2?.candidates?.[0]?.url) thumbnails.push(item.image_versions2.candidates[0].url);
          });
        }
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => { if (e.key === "Enter") handleFetchThumbnail(); };
    ...
    <div className="group relative overflow-hidden rounded-xl border border-slate-200/80 bg-white/90 p-4 backdrop-blur-xl transition-all duration-300 hover:border-slate-300 hover:shadow-lg dark:border-slate-800/80 dark:bg-slate-900/90 dark:hover:border-slate-700">
      ...
      <div className="relative space-y-4">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="text-sm font-bold text-slate-900 dark:text-white">Thumbnail Downloader</h3>
            <p className="text-[10px] text-slate-500 dark:text-slate-400">Extract HD thumbnails</p>
          </div>
        </div>
        ...
        <div className="flex gap-2">
          <div className="relative flex-1">
            <Link2 className="absolute z-50 left-2.5 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
            <Input
              placeholder="Paste post/reel URL"
              value={url}
              onChange={(e) => setUrl(e.target.value)}
              onKeyDown={handleKeyDown}
              className="h-9 rounded-lg border-slate-200/80 bg-white/80 pl-8 pr-3 text-xs backdrop-blur-sm focus:border-amber-400 dark:border-slate-700/80 dark:bg-slate-800/80 dark:text-slate-300"
            />
          </div>
          <Button onClick={handleFetchThumbnail} disabled={loading} size="sm" className="h-9 rounded-lg bg-gradient-to-r from-amber-600 to-orange-600 px-3 text-xs font-medium text-white shadow-md hover:from-amber-700 hover:to-orange-700">
        ...
        {thumbnails.length > 0 && (
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-[11px] text-slate-600 dark:text-slate-400">{thumbnails.length} thumbnails</span>
              <span className="inline-flex items-center gap-1 rounded-full bg-amber-100 px-1.5 py-0.5 text-[10px] text-amber-600 dark:bg-amber-900/50 dark:text-amber-400">
                <Image className="h-2.5 w-2.5" />HD
              </span>
            </div>
            <div className="grid grid-cols-3 gap-2">
              {thumbnails.map((thumb, index) => (
                <div key={index} className="group/thumb relative overflow-hidden rounded-lg border border-slate-200/80 bg-slate-100 dark:border-slate-700/80 dark:bg-slate-800">
                  <div className="absolute left-1 top-1 z-10 rounded bg-black/60 px-1 py-0.5 text-[10px] text-white">#{index + 1}</div>
                  <div className="relative aspect-video overflow-hidden">
                    <img src={thumb} alt={`Thumbnail ${index + 1}`} className="h-full w-full object-cover transition-transform duration-500 group-hover/thumb:scale-110" loading="lazy" />
                  </div>
                  <div className="absolute inset-0 flex items-center justify-center gap-2 bg-gradient-to-t from-amber-600/80 to-transparent opacity-0 transition-all duration-300 group-hover/thumb:opacity-100">
                    <button onClick={() => handleDownload(thumb, index)} className="rounded-full bg-white p-1.5 shadow-lg hover:bg-white/90">
        ...
        {!loading && thumbnails.length === 0 && (
          <div className="flex flex-col items-center justify-center py-6 text-center">
            ...
            <p className="text-[11px] text-slate-400 dark:text-slate-500">Paste URL to extract thumbnails</p>
          </div>
        )}
      </div>
